...
# Load model
mode = 'train'  # Change to 'train'/'load'
saveDir = './models'
...
if mode == 'train':
    ...
    model_conv.load_state_dict(torch.load(os.path.join(saveDir, "resnet18Model.pth")))
...
def displayConvFilters(model_children):
    model_weights = []
    conv_layers = []
...
# Display total number of params.
...
if mode == 'train':
    ...
    torch.save(model_conv.state_dict(), os.path.join(saveDir, "resnet18Model.pth"))

# Test model, also displaying confusion matrix
test_model(model_conv)

# Show conv. filters
model_children = list(model_conv.children())
displayConvFilters(model_children)
